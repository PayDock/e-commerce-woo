...
class CardProcessor
{
    ...
    public function getStandalone3dsToken(): string
    {
        ...
        if (!empty($threeDsCharge['error']) || empty($threeDsCharge['resource']['data']['_3ds']['token'])) {
            ...
            throw new Exception(__('The 3ds charge could not be created successfully.', PAY_DOCK_TEXT_DOMAIN));
        }
    ...
    {
        switch (true) {
            case($this->args['card3ds'] === DSTypes::IN_BUILD(
                )->name && $this->args['cardfraud'] === FraudTypes::IN_BUILD()->name):
                $result = $this->fraud3DsInBuildCharge();
                break;
            case($this->args['card3ds'] === DSTypes::STANDALONE(
                )->name && $this->args['cardfraud'] === FraudTypes::STANDALONE()->name):
    ...
    private function customerCharge(): array
    {
        if ($this->customerId === null) {
            ...
            if ($this->args['cardsavecardoption'] === SaveCardOptions::WITH_GATEWAY(
                )->name && !empty($this->args['gatewayid'])) {
            ...
            if (!empty($customer['error']) || empty($customer['resource']['data']['_id'])) {
                ...
                throw new Exception(__('The Paydock customer could not be created successfully.', PAY_DOCK_TEXT_DOMAIN));
            }
        ...
        if (!empty($responce['error'])) {
            ...
            throw new Exception(__('The charge could not be created successfully.', PAY_DOCK_TEXT_DOMAIN));
        }
    ...
    public function createCustomer($force = false): void
    {
        ...
        if (!empty($customer['error']) || empty($customer['resource']['data']['_id'])) {
            ...
            throw new Exception(__('The Paydock customer could not be created successfully.'.$message, PAY_DOCK_TEXT_DOMAIN));
        }
        $this->logger->createLogRecord(
            $customer['resource']['data']['_id'],
            'Create customer',
