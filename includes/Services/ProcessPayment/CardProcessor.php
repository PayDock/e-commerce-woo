...
class CardProcessor {
	...
	public function run( $order ): array {
		...
		if ( ! in_array( $this->runMethod, self::ALLOWED_METHODS ) ) {
			throw new Exception( __( 'Undefined run method', 'power_board' ) );
		}
	...
	public function getStandalone3dsToken(): string {
		...
		if ( ! empty( $threeDsCharge['error'] ) || empty( $threeDsCharge['resource']['data']['_3ds']['token'] ) ) {
			...
			throw new Exception( __( 'The 3ds charge could not be created successfully.', 'power_board' ) );
		}
	...
	private function customerCharge(): array {
		if ( null === $this->customerId ) {
			...
			if ( ! empty( $customer['error'] ) || empty( $customer['resource']['data']['_id'] ) ) {
				...
				throw new Exception( __( 'The PowerBoard customer could not be created successfully.', 'power_board' ) );
			}
		...
		if ( ! empty( $responce['error'] ) ) {
			...
			throw new Exception( __( 'The charge could not be created successfully.', 'power_board' ) );
		}
	...
	public function createCustomer( $force = false ): void {
		...
		if ( ! empty( $customer['error'] ) || empty( $customer['resource']['data']['_id'] ) ) {
			...
			throw new Exception( __( 'The PowerBoard customer could not be created successfully.' . $message, 'power_board' ) );
		}
		$this->logger->createLogRecord(
			$customer['resource']['data']['_id'],
			'Create customer',
